// Registration.tsx - النسخة المحسنة مع الحقول الجديدة
import React, { useState, useEffect, useRef, useCallback } from "react";
import { useMutation, useQuery } from "convex/react";
import { api } from "../../convex/_generated/api";
import { toast } from "sonner";
import { Upload, User, LogIn, Loader, Camera, Sparkles, Cross, Phone, Calendar } from "lucide-react";
...
export default function Registration({ deviceFingerprint, ipAddress }: RegistrationProps) {
  ...
  useEffect(() => {
    ...
    if (phone.trim() && !/^01[0-2,5]{1}[0-9]{8}$/.test(phone.trim())) {
    ...
    if (birthDate) {
      ...
      minDate.setFullYear(today.getFullYear() - 100); // 100 سنة كحد أقصى
      const maxDate = new Date();
      maxDate.setFullYear(today.getFullYear() - 5); // 5 سنوات كحد أدنى

      if (selectedDate > maxDate) {
        errors.birthDate = "يجب أن يكون العمر 5 سنوات على الأقل";
  ...
  }, [name, existingUser, phone, birthDate]);
  ...
  const validateForm = useCallback(() => {
    ...
    if (!/^01[0-2,5]{1}[0-9]{8}$/.test(phone.trim())) {
    ...
    if (selectedDate > maxDate) {
      toast.error("يجب أن يكون العمر 5 سنوات على الأقل");
      return false;
    }
  ...
  // حساب الحد الأقصى والأدنى لتاريخ الميلاد
  ...
  const { max: maxDate, min: minDate } = getMinMaxDates();
      ...
      <div className="relative w-full max-w-xs mx-auto z-10 px-2">
        ...
        <div className="bg-white/95 backdrop-blur-lg rounded-xl shadow-lg p-4 border border-white/20 transform transition-all duration-300 active:scale-95">
          ...
          <div className="flex flex-col items-center text-center mb-4">
            ...
            <p className="text-gray-600 text-xs mt-0.5">سجّل بياناتك للمشاركة في المسابقة</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div>
              ...
              />
              {formErrors.phone && (
                <p id="phone-error" className="text-red-500 text-xs mt-0.5 flex items-center gap-0.5">
                  ⚠️ {formErrors.phone}
                </p>
              )}
              <p className="text-xs text-gray-500 mt-0.5">رقم الهاتف يجب أن يبدأ بـ 010، 011، 012، 015</p>
            </div>
            ...
            <div>
              ...
              />
              {formErrors.birthDate && (
                <p id="birthDate-error" className="text-red-500 text-xs mt-0.5 flex items-center gap-0.5">
                  ⚠️ {formErrors.birthDate}
                </p>
              )}
              <p className="text-xs text-gray-500 mt-0.5">يجب أن يكون العمر 5 سنوات على الأقل</p>
            </div>
